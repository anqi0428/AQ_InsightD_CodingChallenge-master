...
'''
File_Name: antif_2nd_final_ver.py
...
Functions built are to justify whether there's 2nd degree friendship between the users before they transfer money to each other.
'''
...
for i in range(outputall):
  ...
  id_1 = int(id1[i])
  id_2 = int(id2[i])
  if degree_2(g, id_1, id_2):
    output_2.append('trusted')
  else: 
    output_2.append('unverified')
    g.add_node(id_1)
    g.add_node(id_2)
    g.add_edge(id_1,id_2)

dff = pd.DataFrame(output_2)
dff.to_csv('./paymo_output/output2.txt',header=False, index=False)
